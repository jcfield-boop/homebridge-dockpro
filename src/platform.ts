    ...
    try {
      ...
      for (const device of devices) {
        ...
        if (this.config.devices) {
          const override = (this.config.devices as any[]).find(d => d.id === device.id);
          if (override && override.name) {
            customName = override.name;
            this.log.debug(`Using custom name for device ${device.id}: ${customName}`, LogContext.PLATFORM);
          }
        }
        ...
        if (existingAccessory) {
          ...
          if (existingAccessory.displayName !== customName) {
            existingAccessory.displayName = customName;
            this.log.debug(`Updated accessory name to: ${customName}`, LogContext.PLATFORM);
          }
          ...
          this.log.info(`Adding new accessory: ${customName} (ID: ${device.id})`, LogContext.PLATFORM);
          
          const accessory = new this.homebridgeApi.platformAccessory(customName, uuid);
